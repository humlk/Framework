...
public class RequestParams {
    ...
    private String mUrl;
    private final HashMap mUrlParams = new LinkedHashMap();
    ...
    public void putHeaders(final String key, final Object value) {
     ...
     */
    public void put(String key, final byte[] rawData) {
        hasFile = true;
        writeToOutputStream(key, rawData, TYPE_OCTET_STREAM,
                BINARY_ENCODING, "saiFile");
    }

    public void put(String key, String fileName, final byte[] rawData) {
        hasFile = true;
        writeToOutputStream(key, rawData, TYPE_OCTET_STREAM,
                BINARY_ENCODING, fileName);
    }
    ...
    public void put(final String key, final File file) {
        try {
            ...
            LogUtil.show("HttpParams.put()-> file not found");
        }
    }
    ...
    private void complate(OutputStream outstream) throws IOException {
        if (hasFile) {
            // 参数最末尾的结束符
